...
async def debug_hr_intent_v2():
    ...
    # Имитируем ОБНОВЛЕННЫЙ фильтр FilteredRAGTool для hr_policy
    should_conditions = [
    ...
    # ТЕПЕРЬ ПЕРЕДАЕМ intent="hr_policy"
    results = await hybrid.search(query, limit=10, company_id="ПТФК Технотрон", qdrant_filter=qdrant_filter, intent="hr_policy")
...
if __name__ == "__main__":
    asyncio.run(debug_hr_intent_v2())
