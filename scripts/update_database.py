"""
Скрипт для обновления векторной базы знаний.
"""
...
async def main():
    ...
    # 2. Загрузка чанков с кэшированием
    ...
    if config.use_incremental_updates:
        ...
        await embedding_service.incremental_update(chunks, target_sources=[str(f.relative_to(data_dir)).replace("\\", "/") for f in files])
        for f in files:
